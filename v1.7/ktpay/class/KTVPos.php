...
class KTVPos
{
    ...
    function send_request($params)
    {
        try {
            ...
            curl_setopt($ch, CURLOPT_URL, $params['url']); // set url to post to
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, $params['data']); // add POST fields
            ...
            if ($result = curl_exec($ch)) { // run the whole process
                ...
                return array(
                    'success'=>true,
                    'data'=> $this->get_response_data($params['returnType'], $result)
                );
            }
        } catch (\Throwable $th) {
            return array(
    ...
    function get_response_data($responseType, $response)
    {
        switch ($responseType) {
            case 'json':
    ...
    function check_installment_definition($environment, $merchant_id){
        try 
        {
            ...
            if($response!=null && $response['success'] && $response['data']->success && count($response['data']->value)>0)
            {
                return array(
            ...
            {
                return array(
                    'data'=> "<script type='text/javascript'>alert('Taksit tanımı kontrol edilemedi');</script>",
                    'message' => $response['message']??'Taksit tanımı kontrol edilemedi',
                    'success' => false
                );
            }
        } catch (Exception $e) {
